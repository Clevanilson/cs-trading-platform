...
func TestWithdraw(t *testing.T) {
	...
	setup := func() {
		...
		wallet, err = entity.NewWallet(entity.WalletBuilder{AccountID: "Dante"})
		err = wallet.Deposit("USD", 500)
	...
	t.Run("With valid value", func(t *testing.T) {
		...
		pkgassert.Equals(t, wallet.Balances()[0].Amount(), 0)
	})
	...
	t.Run("With invalid value", func(t *testing.T) {
		...
		wallet, err = repository.GetByAccountID(wallet.AccountID())
		pkgassert.Equals(t, wallet.Balances()[0].Amount(), 500)
	})
}
